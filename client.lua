...
AddEventHandler('esx:playerLoaded', function(xPlayer)
  ...
  if gang.name ~= 'nogang' then
    if gang.name == 'Sheriff' then
      SendNUIMessage({action = "gang", value = string.gsub(gang.name, "_", " ") .. " | " .. gang.grade_label, icon = string.lower(gang.name)})
    else
      SendNUIMessage({action = "gang", value = string.gsub(gang.name, "_", " ") .. " | " .. gang.grade_label, icon = 'gang'})
    end
  end
  if string.lower(job.name) ~= 'nojob' and (string.lower(job.name) ~= 'police' or job.ext == nil) then
    if job.grade > 0 then
      ...
      SendNUIMessage({action = "job", value = job.label .. " | " .. 'Off-Duty', icon = job.name})
    end    
...
RegisterCommand("mmggff", function()
