...
AddEventHandler("saveHungerThirst", function(hunger, thirst)
  ...
  TriggerEvent('es:getPlayerFromId', _source, function(user)
		...
		exports.oxmysql:execute("UPDATE users SET status=@status WHERE idSteam=@identifier", {['@identifier'] = player, ['@status'] = {['hunger']=hunger,['thirst']=thirst}})
	end)
end)
...
AddEventHandler("getPlayerStatus", function()
  ...
  TriggerEvent('es:getPlayerFromId', _source, function(user)
    local player = user.getIdentifier()
    exports.oxmysql:execute('SELECT status FROM users WHERE identifier = @identifier', {['@identifier'] = player}, function(result)
